...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'guaran_package/new_package',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
            ...
            //  alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"guaran_package/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'guaran_package/update_role',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
function delete_roles(q_id)
{
   ...
   if(confirm("Bạn có muốn xóa gói bảo hành này đi?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
	   ...
	   if(result=="success")
				{
					...
					$('#example2').DataTable().ajax.reload();
				  	...
				  	toastr["error"]("Xóa thất bại ! Thử lại sau");
				}
				else{
					toastr["error"](result);
				}
