...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"kinds";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'kinds/update_unit',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	var base_url=$("#base_url").val().trim();
	$.post(base_url+"kinds/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
function delete_unit(q_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"kinds/delete_unit",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
