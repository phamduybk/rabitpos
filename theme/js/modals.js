...
$(".add_kind").on("click", function (e) {
  ...
  if (confirm("Are you Sure ?")) {
    e.preventDefault();
    data = new FormData($('#kind_form')[0]);//form name
    /*Check XSS Code*/
    if (!xss_validation(data)) { return false; }
    ...
    $.ajax({
      type: 'POST',
      url: base_url + 'kinds/add_kind_modal',
      data: data,
      cache: false,
      contentType: false,
      processData: false,
      success: function (result) {
        ...
        var data = jQuery.parseJSON(result);
        if (data.result == "success") {
          $('#kind_modal').modal('toggle');
          var newOption = '<option value=' + data.id + ' selected>' + data.kind_name + '</option>';
          $('#kind_id').append(newOption).trigger('change');
          //$("#amount").val(data.advance);
          $('#kind_form')[0].reset();
          toastr["success"]("New kind Added!!");
          success.currentTime = 0;
          success.play();
        }
        else if (data.result == "failed") {
